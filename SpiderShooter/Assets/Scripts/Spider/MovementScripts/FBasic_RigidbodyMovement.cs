...
namespace SpiderShooter.Spider.FBasics
{
    /// <summary>
    ...
    public class FBasic_RigidbodyMovement : FBasic_CharacterHorizontalBase
    {
        ...
        protected override void FixedUpdate()
        {
            if (Grounded)
            {
                ...
                Vector3 newVelocity =
                    new(
                        lastTargetVelocityForward.x * accForw,
                        CharacterRigidbody.velocity.y,
                        lastTargetVelocityForward.z * accForw
                    );
                newVelocity += new Vector3(
                    lastTargetVelocityRight.x * accRight,
                    0f,
                    lastTargetVelocityRight.z * accRight
                );
        ...
        protected void CheckGroundPlacement()
        {
            if (!Jumped)
            {
                if (!Grounded)
                {
                    Ray groundRay = new(transform.position + (Vector3.up * 0.2f), Vector3.down);
                    ...
                    )
                    {
                        ...
                        float colliderToTransformHeight =
                        ...
                        transform.position = new Vector3(
                            ...
                            groundHit.point.y - colliderToTransformHeight,
        ...
        protected override void Jump()
        {
            ...
            CharacterRigidbody.MovePosition(
                transform.position + (Vector3.up * fakeYVelocity * Time.fixedDeltaTime)
            );
        ...
        private void OnCollisionStay(Collision collision)
        {
            ...
            {
                if (
                    Vector3.Dot(collision.contacts[i].point - transform.position, transform.up)
                    < dotPrecision
                )
                {
                    Grounded = true;
        ...
        private void CheckIfFacingWall(Collision collision)
        {
            ...
            if (accelerationForward != 0f)
            {
                for (int i = 0; i < collision.contacts.Length; i++)
                {
                    ...
                    if (
                        Vector3.Dot(
                            collision.contacts[i].point - transform.position,
                            transform.forward * verticalSign
                        ) > 0.25f
                    )
                    {
                        FacingWall = true;
            ...
            if (accelerationRight != 0f)
            {
                for (int i = 0; i < collision.contacts.Length; i++)
                {
                    ...
                    if (
                        Vector3.Dot(
                            collision.contacts[i].point - transform.position,
                            transform.right * Mathf.Sign(horizontalValue)
                        ) > 0.25f
                    )
                    {
                        SideingWall = true;
