...
namespace Mirror.Weaver
{
    public class ILPostProcessorHook : ILPostProcessor
    {
        ...
        private ILPostProcessorLogger Log = new();
        ...
        public override bool WillProcess(ICompiledAssembly compiledAssembly)
        {
            ...
            bool relevant =
                compiledAssembly.Name == MirrorRuntimeAssemblyName
        ...
        public override ILPostProcessResult Process(ICompiledAssembly compiledAssembly)
        {
            ...
            using (ILPostProcessorAssemblyResolver asmResolver = new(compiledAssembly, Log))
            {
                ...
                using AssemblyDefinition asmDef = AssemblyDefinition.ReadAssembly(
                    stream,
                    readerParameters
                );
                ...
                if (weaver.Weave(asmDef, asmResolver, out bool modified))
                {
                    ...
                    if (modified)
                    {
                        ...
                        if (
                            asmDef.MainModule.AssemblyReferences.Any(
                                r => r.Name == asmDef.Name.Name
                            )
                        )
                        {
                            _ = asmDef.MainModule.AssemblyReferences.Remove(
                                asmDef.MainModule.AssemblyReferences.First(
                                    r => r.Name == asmDef.Name.Name
                                )
                            );
