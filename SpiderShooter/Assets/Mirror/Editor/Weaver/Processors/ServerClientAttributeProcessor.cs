...
namespace Mirror.Weaver
{
    internal static class ServerClientAttributeProcessor
    {
        ...
        private static bool ProcessSiteMethod(
            WeaverTypes weaverTypes,
            Logger Log,
            MethodDefinition md,
            ref bool WeavingFailed
        )
        {
            ...
            return md.Body != null
                && md.Body.Instructions != null
                && ProcessMethodAttributes(weaverTypes, md);
        }
        ...
        private static void InjectServerGuard(
            WeaverTypes weaverTypes,
            MethodDefinition md,
            bool logWarning
        )
        {
            ...
            worker.InsertBefore(
                top,
                worker.Create(OpCodes.Call, weaverTypes.NetworkServerGetActive)
            );
            ...
            if (logWarning)
            {
                ...
                worker.InsertBefore(
                    top,
                    worker.Create(OpCodes.Call, weaverTypes.logWarningReference)
                );
            }
        ...
        private static void InjectClientGuard(
            WeaverTypes weaverTypes,
            MethodDefinition md,
            bool logWarning
        )
        {
            ...
            worker.InsertBefore(
                top,
                worker.Create(OpCodes.Call, weaverTypes.NetworkClientGetActive)
            );
            worker.InsertBefore(top, worker.Create(OpCodes.Brtrue, top));
            if (logWarning)
            {
        ...
        private static void InjectGuardParameters(
            MethodDefinition md,
            ILProcessor worker,
            Instruction top
        )
        {
            ...
            for (int index = 0; index < md.Parameters.Count; index++)
            {
                ...
                if (param.IsOut)
                {
                    ...
                    worker.InsertBefore(
                        top,
                        worker.Create(OpCodes.Ldloca_S, (byte)(md.Body.Variables.Count - 1))
                    );
                    worker.InsertBefore(top, worker.Create(OpCodes.Initobj, elementType));
                    worker.InsertBefore(
                        top,
                        worker.Create(OpCodes.Ldloc, md.Body.Variables.Count - 1)
                    );
        ...
        private static void InjectGuardReturnValue(
            MethodDefinition md,
            ILProcessor worker,
            Instruction top
        )
