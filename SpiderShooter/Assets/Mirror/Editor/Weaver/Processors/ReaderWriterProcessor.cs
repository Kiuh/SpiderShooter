...
namespace Mirror.Weaver
{
    public static class ReaderWriterProcessor
    {
        ...
        private static void ProcessMirrorAssemblyClasses(
            AssemblyDefinition CurrentAssembly,
            IAssemblyResolver resolver,
            Logger Log,
            Writers writers,
        ...
        )
        {
            ...
            AssemblyNameReference mirrorAssemblyReference =
                CurrentAssembly.MainModule.FindReference(Weaver.MirrorAssemblyName);
            ...
            {
                Log.Error(
                    "Failed to find Mirror AssemblyNameReference. Can't register Mirror.dll readers/writers."
                );
            }
        }

        private static bool ProcessAssemblyClasses(
            AssemblyDefinition CurrentAssembly,
        ...
        private static bool LoadMessageReadWriter(
            ModuleDefinition module,
            Writers writers,
            Readers readers,
            TypeDefinition klass,
            ref bool WeavingFailed
        )
        {
            bool modified = false;
            if (
                !klass.IsAbstract
                && !klass.IsInterface
                && klass.ImplementsInterface<NetworkMessage>()
            )
            {
                _ = readers.GetReadFunc(module.ImportReference(klass), ref WeavingFailed);
                _ = writers.GetWriteFunc(module.ImportReference(klass), ref WeavingFailed);
                modified = true;
            }
        ...
        private static bool LoadDeclaredWriters(
            AssemblyDefinition currentAssembly,
            TypeDefinition klass,
            Writers writers
        )
        {
            ...
            foreach (MethodDefinition method in klass.Methods)
            {
                ...
                if (
                    !method.HasCustomAttribute<System.Runtime.CompilerServices.ExtensionAttribute>()
                )
                {
                    continue;
                }
        ...
        private static bool LoadDeclaredReaders(
            AssemblyDefinition currentAssembly,
            TypeDefinition klass,
            Readers readers
        )
        {
            ...
            foreach (MethodDefinition method in klass.Methods)
            {
                ...
                readers.Register(
                    method.ReturnType,
                    currentAssembly.MainModule.ImportReference(method)
                );
                modified = true;
            }
            return modified;
        ...
        private static void AddRuntimeInitializeOnLoadAttribute(
            AssemblyDefinition assembly,
        ...
        )
        {
            ...
            MethodDefinition ctor = weaverTypes.runtimeInitializeOnLoadMethodAttribute
                .GetConstructors()
                .Last();
            ...
            CustomAttribute attribute = new(assembly.MainModule.ImportReference(ctor));
            // add the RuntimeInitializeLoadType.BeforeSceneLoad argument to ctor
            attribute.ConstructorArguments.Add(
                new CustomAttributeArgument(
                    weaverTypes.Import<RuntimeInitializeLoadType>(),
        ...
        private static void AddInitializeOnLoadAttribute(
            AssemblyDefinition assembly,
            WeaverTypes weaverTypes,
            MethodDefinition method
        )
        {
            ...
            MethodDefinition ctor = weaverTypes.initializeOnLoadMethodAttribute
                .GetConstructors()
                .First();
            // using ctor directly throws: ArgumentException: Member 'System.Void UnityEditor.InitializeOnLoadMethodAttribute::.ctor()' is declared in another module and needs to be imported
            // we need to import it first.
        ...
        public static void InitializeReaderAndWriters(
            AssemblyDefinition currentAssembly,
            WeaverTypes weaverTypes,
            Writers writers,
            Readers readers,
        ...
        {
            MethodDefinition initReadWriters =
                new(
                    "InitReadWriters",
