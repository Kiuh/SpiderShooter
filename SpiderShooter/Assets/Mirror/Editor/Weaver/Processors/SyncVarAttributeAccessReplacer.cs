...
namespace Mirror.Weaver
{
    public static class SyncVarAttributeAccessReplacer
    {
        ...
        )
        {
            ...
            Console.WriteLine(
                $"  ProcessSitesModule {moduleDef.Name} elapsed time:{DateTime.Now - startTime}"
            );
        }
        ...
        )
        {
            ...
            if (instr.OpCode == OpCodes.Stfld)
            {
                ...
                else if (instr.Operand is FieldReference opFieldstRef)
                {
                    ...
                    if (field.HasCustomAttribute<SyncVarAttribute>())
                    {
                        ...
                        Log.Error(
                            $"'[SyncVar] {opFieldstRef.Name}' in '{md.Module.Name}' is modified by '{md.FullName}' in '{field.Module.Name}'. Modifying a [SyncVar] from another assembly is not supported. Please add a: 'public void Set{opFieldstRef.Name}(value) {{ this.{opFieldstRef.Name} = value; }}' function in '{opFieldstRef.DeclaringType.Name}' and call this function from '{md.FullName}' instead."
                        );
                    }
                }
            }
            ...
            if (instr.OpCode == OpCodes.Ldflda)
            {
                ...
                if (instr.Operand is FieldDefinition opFieldlda)
                {
                    ...
                    return ProcessLoadAddressInstruction(
                        syncVarAccessLists,
                        md,
                        instr,
                        opFieldlda,
        ...
        private static void ProcessSetInstruction(
            SyncVarAccessLists syncVarAccessLists,
            MethodDefinition md,
            Instruction i,
            FieldDefinition opField
        )
        {
            ...
            if (
                syncVarAccessLists.replacementSetterProperties.TryGetValue(
                    opField,
                    out MethodDefinition replacement
                )
            )
            {
        ...
        private static void ProcessGetInstruction(
            SyncVarAccessLists syncVarAccessLists,
            MethodDefinition md,
            Instruction i,
            FieldDefinition opField
        )
        {
            ...
            // does it set a field that we replaced?
            if (
                syncVarAccessLists.replacementGetterProperties.TryGetValue(
                    opField,
                    out MethodDefinition replacement
                )
            )
        ...
        private static int ProcessLoadAddressInstruction(
            SyncVarAccessLists syncVarAccessLists,
            MethodDefinition md,
            Instruction instr,
            FieldDefinition opField,
        ...
        )
        {
            ...
            )
            {
                ...
                if (nextInstr.OpCode == OpCodes.Initobj)
                {
                    ...
                    VariableDefinition tmpVariable = new(opField.FieldType);
