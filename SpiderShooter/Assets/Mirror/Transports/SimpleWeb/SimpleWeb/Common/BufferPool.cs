...
namespace Mirror.SimpleWeb
{
    ...
    public sealed class ArrayBuffer : IDisposable
    {
        ...
        public void CopyTo(byte[] target, int offset)
        {
            if (count > (target.Length + offset))
            {
                throw new ArgumentException(
                    $"{nameof(count)} was greater than {nameof(target)}.length",
                    nameof(target)
                );
            }
        ...
        public void CopyFrom(byte[] source, int offset, int length)
        {
            ...
            {
                throw new ArgumentException(
                    $"{nameof(length)} was greater than {nameof(array)}.length",
                    nameof(length)
                );
            }
        ...
        internal void Validate(int arraySize)
        {
            ...
            {
                Log.Error(
                    "[SimpleWebTransport] Buffer that was returned had an array of the wrong size"
                );
            }
        }
    }
    ...
    internal class BufferBucket : IBufferOwner
    {
        ...
        {
            _ = Interlocked.Increment(ref _current);
    ...
    public class BufferPool
    {
        ...
        private void Validate()
        {
            ...
            {
                Log.Error(
                    $"[SimpleWebTransport] BufferPool Failed to create bucket for smallest. bucket:{buckets[0].arraySize} smallest{smallest}"
                );
            }
            ...
            {
                Log.Error(
                    $"[SimpleWebTransport] BufferPool Failed to create bucket for largest. bucket:{largestBucket} smallest{largest}"
                );
            }
        }
