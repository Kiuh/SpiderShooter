    ...
    internal class ServerHandshake
    {
        ...
        public bool TryHandshake(Connection conn)
        {
            ...
            using (ArrayBuffer getHeader = bufferPool.Take(GetSize))
            {
                ...
                {
                    Log.Warn(
                        $"[SimpleWebTransport] First bytes from client was not 'GET' for handshake, instead was {Log.BufferToString(getHeader.array, 0, GetSize)}",
                        false
                    );
                    return false;
                }
        ...
        private string ReadToEndForHandshake(Stream stream)
        {
            ...
            int? readCountOrFail = ReadHelper.SafeReadTillMatch(
                stream,
                readBuffer.array,
                0,
                maxHttpHeaderSize,
        ...
        {
            using ArrayBuffer keyBuffer = bufferPool.Take(
                    KeyLength + Constants.HandshakeGUIDLength
                ),
        ...
        {
            int start =
                msg.IndexOf(KeyHeaderString, StringComparison.InvariantCultureIgnoreCase)
                + KeyHeaderString.Length;

            Log.Verbose(
                $"[SimpleWebTransport] Handshake Key: {msg.Substring(start, KeyLength)}",
                false
            );
            _ = Encoding.ASCII.GetBytes(msg, start, KeyLength, keyBuffer, 0);
        }

        private static void AppendGuid(byte[] keyBuffer)
        {
            Buffer.BlockCopy(
                Constants.HandshakeGUIDBytes,
                0,
                keyBuffer,
                KeyLength,
                Constants.HandshakeGUIDLength
            );
        }
        ...
        {
            Log.Verbose(
                $"[SimpleWebTransport] Handshake Hashing {Encoding.ASCII.GetString(keyBuffer, 0, MergedKeyLength)}",
                false
            );
        ...
        private static void CreateResponse(byte[] keyHash, byte[] responseBuffer)
        {
            ...
            Log.Verbose(
                $"[SimpleWebTransport] Handshake Response length {message.Length}, IsExpected {message.Length == ResponseLength}",
                false
            );
            _ = Encoding.ASCII.GetBytes(message, 0, ResponseLength, responseBuffer, 0);
        }
    }
}
