...
namespace Telepathy
{
    public class Server : Common
    {
        ...
        private readonly ConcurrentDictionary<int, ConnectionState> clients = new();
        ...
        private void Listen(int port)
        {
            ...
            try
            {
                ...
                while (true)
                {
                    ...
                    ConnectionState connection = new(client, MaxMessageSize);
                    ...
                    Thread receiveThread =
                        new(() =>
                        {
                            ...
                            {
                                Log.Error(
                                    "[Telepathy] Server client thread exception: " + exception
                                );
                            }
                        })
                        {
        ...
        public bool Send(int connectionId, ArraySegment<byte> message)
        {
            ...
            if (message.Count <= MaxMessageSize)
            {
                ...
                if (clients.TryGetValue(connectionId, out ConnectionState connection))
                {
                    ...
                    if (connection.sendPipe.Count < SendQueueLimit)
                    {
                        ...
                        _ = connection.sendPending.Set(); // interrupt SendThread WaitOne()
                        return true;
                    }
                    // disconnect if send queue gets too big.
                    ...
                    else
                    {
                        // log the reason
                        Log.Warning(
                            $"[Telepathy] Server.Send: sendPipe for connection {connectionId} reached limit of {SendQueueLimit}. This can happen if we call send faster than the network can process messages. Disconnecting this connection for load balancing."
                        );
            ...
            Log.Error(
                "[Telepathy] Server.Send: message too big: "
                    + message.Count
                    + ". Limit: "
                    + MaxMessageSize
            );
        ...
        {
            // find the connection
            return clients.TryGetValue(connectionId, out ConnectionState connection)
                ? ((IPEndPoint)connection.client.Client.RemoteEndPoint).Address.ToString()
                : "";
        }
