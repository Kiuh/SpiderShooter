...
namespace Telepathy
{
    public static class ThreadFunctions
    {
        ...
        public static bool SendMessagesBlocking(
        ...
        )
        {
            ...
            {
                Log.Error(
                    $"[Telepathy] ReadMessageBlocking: payloadBuffer needs to be of size 4 + MaxMessageSize = {4 + MaxMessageSize} instead of {payloadBuffer.Length}"
                );
                return false;
            }
            ...
            Log.Warning(
                "[Telepathy] ReadMessageBlocking: possible header attack with a header of: "
                    + size
                    + " bytes."
            );
            return false;
        ...
        )
        {
            ...
            try
            {
                ...
                while (true)
                {
                    ...
                    ArraySegment<byte> message = new(receiveBuffer, 0, size);
                    ...
                    if (receivePipe.Count(connectionId) >= QueueLimit)
                    {
                        // log the reason
                        Log.Warning(
                            $"[Telepathy] ReceivePipe reached limit of {QueueLimit} for connectionId {connectionId}. This can happen if network messages come in way faster than we manage to process them. Disconnecting this connection for load balancing."
                        );
            ...
            catch (Exception exception)
            {
                ...
                Log.Info(
                    "[Telepathy] ReceiveLoop finished receive function for connectionId="
                        + connectionId
                        + " reason: "
                        + exception
                );
        ...
        )
        {
            ...
            try
            {
                while (client.Connected) // try this. client will get closed eventually.
                {
                    ...
                    //    the next Send call.
                    _ = sendPending.Reset(); // WaitOne() blocks until .Set() again
            ...
            catch (Exception exception)
            {
                ...
                // -> either way we should stop gracefully
                Log.Info(
                    "[Telepathy] SendLoop Exception: connectionId="
                        + connectionId
                        + " reason: "
                        + exception
                );
