...
namespace kcp2k
{
    ...
    public class KcpTransport : Transport, PortTransport
    {
        ...
        [Tooltip(
            "KCP reliable max message size shown for convenience. Can be changed via ReceiveWindowSize."
        )]
        [ReadOnly]
        ...
        {
            return channel == KcpChannel.Reliable ? Channels.Reliable : Channels.Unreliable;
        }

        public static KcpChannel ToKcpChannel(int channel)
        {
            return channel == Channels.Reliable ? KcpChannel.Reliable : KcpChannel.Unreliable;
        }
        ...
        public override void ClientConnect(Uri uri)
        {
            if (uri.Scheme != Scheme)
            {
                throw new ArgumentException(
                    $"Invalid url {uri}, use {Scheme}://host:port instead",
                    nameof(uri)
                );
            }
        ...
        public override int GetMaxPacketSize(int channelId = Channels.Reliable)
        {
            ...
            {
                Channels.Unreliable => KcpPeer.UnreliableMaxMessageSize(config.Mtu),
                _ => KcpPeer.ReliableMaxMessageSize(config.Mtu, ReceiveWindowSize),
            };
        }
        ...
        public long GetAverageMaxSendRate()
        {
            return server.connections.Count > 0
                ? server.connections.Values.Sum(conn => conn.peer.MaxSendRate)
        ...
        {
            return server.connections.Count > 0
                ? server.connections.Values.Sum(conn => conn.peer.MaxReceiveRate)
                    / server.connections.Count
                : 0;
        }
        ...
        {
            return server.connections.Values.Sum(conn => conn.peer.SendQueueCount);
        }

        private long GetTotalReceiveQueue()
        {
            return server.connections.Values.Sum(conn => conn.peer.ReceiveQueueCount);
        }

        private long GetTotalSendBuffer()
        {
            return server.connections.Values.Sum(conn => conn.peer.SendBufferCount);
        }

        private long GetTotalReceiveBuffer()
        {
            return server.connections.Values.Sum(conn => conn.peer.ReceiveBufferCount);
        }
