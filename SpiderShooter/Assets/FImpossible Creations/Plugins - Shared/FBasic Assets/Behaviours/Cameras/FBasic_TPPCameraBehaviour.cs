...
namespace FIMSpace.Basics
{
    ...
    public class FBasic_TPPCameraBehaviour : MonoBehaviour
    {
        ...
        private Vector3 movVelo = Vector3.zero;
        ...
        /// <summary>
        /// Calculations for input mouse control
        /// </summary>
        ...
        {
            targetSphericRotation.y = HelperClampAngle(
                targetSphericRotation.y,
                RotationRanges.x,
                RotationRanges.y
            );

            animatedSphericRotation =
                RotationSpeed < 1f
                    ? new Vector2(
                        Mathf.LerpAngle(
                            animatedSphericRotation.x,
                            targetSphericRotation.x,
                            Time.deltaTime * 30 * RotationSpeed
                        ),
                        Mathf.LerpAngle(
                            animatedSphericRotation.y,
                            targetSphericRotation.y,
                            Time.deltaTime * 30 * RotationSpeed
                        )
                    )
                    : targetSphericRotation;
            ...
            if (HardFollowValue < 1f)
            {
                targetPosition = Vector3.SmoothDamp(
                    ...
                    targetPosition,
                    ref movVelo,
                    Mathf.Lerp(.5f, 0f, HardFollowValue),
                    Mathf.Infinity,
                    Time.deltaTime
                );
            }
        ...
        private void RaycastCalculations()
        {
            Vector3 followPoint =
                ...
                + FollowingOffset
                + transform.TransformVector(FollowingOffsetDirection);
            Quaternion cameraDir = Quaternion.Euler(
                targetSphericRotation.y,
                targetSphericRotation.x,
                0f
            );
            ...
            {
                transform.position =
                    sightObstacleHit.point - (directionRay.direction * CollisionOffset);
            }
            else
            {
