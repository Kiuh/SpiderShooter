...
namespace AYellowpaper.Editor
{
    internal class ObjectSelectorWindow : EditorWindow
    {
        ...
        private static ItemInfo _nullItem = new() { InstanceID = null, Label = "None" };
        ...
        private void InitVisualElements()
        {
            ...
            _ = _searchbox.RegisterValueChangedCallback(SearchFilterChanged);
        ...
        {
            if (_showSceneObjects && toggle == _sceneTab)
            {
                return;
            }
            ...
            FilterItems();
            List<int> list = new();
            if (GetIndexOfCurrentItem(out int index))
            {
                list.Add(index);
            }
        ...
        private void BindItem(VisualElement listItem, int index)
        {
            ...
            Label label = listItem.Q<Label>();
            if (label != null)
            {
                label.text = _filteredItems[index].Label;
            }

            Image image = listItem.Q<Image>();
        ...
        {
            GetText(_currentItem, out string infoText, out string indexText, out string typeText);
        ...
        )
        {
            ...
            Object obj = EditorUtility.InstanceIDToObject((int)itemInfo.InstanceID);
            ...
            {
                Transform transform =
                    (obj is GameObject go) ? go.transform : (obj as Component).transform;
                int compIndex = Array.IndexOf(
                    transform.gameObject.GetComponents(typeof(Component)),
                    obj
                );
        ...
        private static string GetTransformPath(Transform transform)
        {
            ...
            _ = sb.Append(transform.name);
            while (transform.parent != null)
            {
                _ = sb.Insert(0, transform.parent.name + "/");
        ...
        {
            HierarchyProperty property = new(HierarchyType.Assets, false);
        ...
        {
            HierarchyProperty property = new(HierarchyType.GameObjects, false);
        ...
        {
            bool? matchFilterConstraint = _filter.SceneFilterCallback?.Invoke(obj);
    ...
    public class ObjectSelectorFilter
    {
        ...
        public ObjectSelectorFilter()
            : this("", x => true) { }

        public ObjectSelectorFilter(
            string assetSearchFilter,
            Func<Object, bool> sceneFilterCallback
        )
        {
